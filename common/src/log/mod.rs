...
pub fn init_logging(log_opts: LoggingOpts) -> anyhow::Result<()> {
    // Logging lib errors and all app logs
    let log_level = LevelFilter::from_str(&log_opts.level)?;
    ...
    let log_level_for: HashMap<String, String> = log_opts.log_level_for.inner;
